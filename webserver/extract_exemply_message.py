...
def get_roots(commands):
	...
	for au_k, au_d in Author_Detail_l:
		au_num += 1
		person={}
		person["Author_Name"] = au_d["AuN"]
		person["AuId"] = au_d["AuId"]
		person["now_year"] = now_year
		person["first_page_year"] = au_d["start_year"]
		person["academic_age"] = now_year - person["first_page_year"]
		person["Fields"] = [ {"Field_Name":FId_to_FN(fid[0]) ,"label_color":FId_to_Color(fid[0]) }  for fid in au_d["work_field"][0:3] ]

		if not person["Fields"]:
			 person["Fields"] = [{"Field_Name": "no_field","label_color":"#CCFFFF"}]

		p_history_af = au_d["history_work_for"]
		p_history_af = sorted(p_history_af.items(), key = lambda d:d[1][1] , reverse=True)[0:3]
		history_dict = {}
		for p in p_history_af:
			history_dict[AfId_to_AfN(p[0])]='-'.join( [str(j) for j in p[1]] )  
		
		person["History_Afflication"] = history_dict 
		person["total_CC"] =    au_d["total_CC"]
		person["now_work_for"] = au_d["now_work_for"]
		...
		for paper in papers:
			child = {}
			child["Title"] = paper[1].replace('"','\'').replace("\\","")  # paper name
			child["Cited Num"] = str(paper[3])  #cited CC
			child["Publish Year"] = str(paper[2])
			Top = False
			if paper[4] and paper[5]:
				child["Publish Journal"] = paper[5]
				Top = JId_is_Top(paper[4])
			elif paper[6] and paper[7]:
				child["Publish Conference"] = paper[7]
		...
		person["first author paper"] = children
...
def output_iter(outf, message):
	if type(message) is dict:
		outf.write("\n")
		for key, value in message.items():
			outf.write(str(key) + ",")
			output_iter(outf, value)
			outf.write("\n")
		...
		outf.write(str(message))



if __name__ =='__main__':
	init_dict()
	roots = get_roots({"score":{"$lt":1000}})	
	print roots	
	for people in roots[9:15]:		
		with open("people/"+people["Author_Name"]+".csv",'w') as outf:
			output_iter(outf, people)
